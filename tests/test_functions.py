...
def test_adsbx_to_cot_raw(sample_feed):
    ...
    cot = zellocot.functions.adsbx_to_cot_raw(sample_craft)
...
def test_adsbx_to_cot(sample_feed):
    ...
    sample_craft = sample_feed["aircraft"][0]
    cot = zellocot.adsbx_to_cot(sample_craft)
    expected = (
        ...
        b'Squawk: None Category: A1 (via zellocot@rorqual)"><UID Droid="ICAO-A9EE47" />'
        b'<contact callsign="N739UL--" /><track course="50.1" speed="40.641076" /><remarks>N739UL-- ICAO: '
        b"A9EE47 REG:  Flight: N739UL Type:  Squawk: None Category: A1 (via zellocot@rorqual)</remarks>"
        b"</detail></event>"
    )
...
def test_adsbx_to_cot_raw_with_known_craft(sample_feed, sample_known_craft):
    ...
    cot = zellocot.functions.adsbx_to_cot_raw(sample_craft, known_craft=known_craft)
